    ...
    eprintln!(
        "Stoke — the firewall for AI agent spend.\n\
         ...
         Commands:\n  \
           ...
           setup    Guided local setup (config + API key)\n  \
...
fn discover_model_name(body: &str) -> Option<String> {
...
fn discover_ollama_model(ollama_url: &str) -> Option<String> {
    let native = ollama_url.trim_end_matches('/').trim_end_matches("/v1");
    ureq::get(&format!("{}/api/tags", native))
        .timeout(std::time::Duration::from_secs(2))
        .call()
        .ok()
        .and_then(|response| response.into_string().ok())
        .and_then(|body| discover_model_name(&body))
}
...
fn render_setup_config(model: &str, ollama_url: &str, bind_all: bool) -> String {
    ...
    format!(
        r#"# Stoke — generated by `stoke setup`
# Load .env before starting: set -a; . ./.env; set +a
routing = "single"
default_model = "{}"
...
fn generate_api_key() -> Result<String, String> {
    ...
    std::fs::File::open("/dev/urandom")
        ...
        .map_err(|error| format!("could not generate API key: {}", error))?;
    Ok(hex::encode(bytes))
}
...
fn write_owner_file(path: &std::path::Path, content: &str) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600))
            .map_err(|error| format!("could not protect {}: {}", path.display(), error))?;
    }
    Ok(())
}

fn cli_setup(args: &[String]) -> ExitCode {
    let mut ollama_url = String::new();
    let mut model = String::new();
    let mut output = "stoke.toml".to_string();
    let mut env_output = ".env".to_string();
            ...
            "--help" | "-h" => {
                eprintln!(
                    "stoke setup — guided local setup\n\n\
                     Detects a local Ollama model, writes stoke.toml, and creates\n\
                     a 0600 .env containing a random STOKE_API_KEYS value.\n\n\
                     Options:\n  \
                       --ollama-url   Ollama OpenAI URL (default: http://127.0.0.1:11434/v1)\n  \
                       --model, -m    Model name (otherwise first model discovered)\n  \
                       --output, -o   Config path (default: stoke.toml)\n  \
                       --env-output   Environment file (default: .env)\n  \
                       --bind-all     Bind to 0.0.0.0 instead of localhost\n  \
    ...
    if ollama_url.is_empty() {
        ollama_url = prompt_line("Ollama URL", "http://127.0.0.1:11434/v1");
    }
    if model.is_empty() {
        if let Some(discovered) = discover_ollama_model(&ollama_url) {
            model = prompt_line("Model", &discovered);
            eprintln!("✓ Discovered {}", discovered);
        } else {
            model = prompt_line("Model", "");
        }
    }
    if model.is_empty() {
        eprintln!(
    ...
    if let Err(error) = write_owner_file(env_path, &format!("STOKE_API_KEYS={}\n", api_key)) {
        eprintln!("Error: {}", error);
        return ExitCode::FAILURE;
    }

    println!("Generated {} and {}", output, env_output);
    println!("\nStart Stoke:");
    println!("  set -a; . {}; set +a; stoke serve", env_output);
    println!("\nTest it:");
    println!("  stoke route -m {} -p 'Hello!'", model);
    ExitCode::SUCCESS
}
...
mod tests {
    ...
    #[test]
    fn setup_config_uses_selected_model_and_server_defaults() {
        let config = render_setup_config("qwen2.5-coder:7b", "http://127.0.0.1:11434/v1", false);
