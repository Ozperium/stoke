    ...
    def test_enabled_owns_augmented_config_and_stops_worker(self):
        with tempfile.TemporaryDirectory(prefix="headroom-launch-test-") as raw:
            ...
            worker_impl.write_text(
                ...
                "    server.listen()\n"
                f"    Path({str(port_dump)!r}).write_text(str(server.getsockname()[1]))\n"
                "    os.write(ready_fd, json.dumps({'port': server.getsockname()[1]}).encode() + b'\\n')\n"
                "    os.close(ready_fd)\n"
                "    while True:\n"
                "        client, _ = server.accept()\n"
                "        request = client.recv(4096)\n"
                "        token = os.environ['STOKE_HEADROOM_TOKEN'].encode()\n"
                "        status = b'200 OK' if b'GET /health ' in request and b'Authorization: Bearer ' + token in request else b'401 Unauthorized'\n"
                "        body = b'{\"status\":\"ok\"}' if status == b'200 OK' else b''\n"
